...
- **修复 `m.replace` 编辑事件处理**：编辑事件现在会获取被替换消息，并以 `[quote] -> [new_text]` 的形式送入 AstrBot；同时保留 `m.new_content` 规范化和 `* ` 前缀清理。
- 修复了开启会话隔离下的发送
...
- 房间密钥与秘密共享现在只接受已验证的同账号设备，正确维护 `m.forwarded_room_key` 转发链和 `withheld`，拒绝未加密 room key，支持同批请求取消、`m.no_olm` 去重及一小时 Olm 恢复限流。
- OAuth2 发现显式跟随 `/.well-known/matrix/client` 的 30x 跳转；Matrix API 访问令牌继续只通过 `Authorization: Bearer` 头发送，不依赖已移除的查询参数方式。
...
- 缺失房间密钥的请求现在会复用 request ID、5 秒节流重试，并在成功导入密钥后发送取消请求；主动交换开启且间隔为 `0` 时采用 30 秒周期，关闭且间隔为 `0` 时保持 lazyload 模式，在加密发送或发现用户新设备时按需交换。
...
- 现在同账户的其他登录设备可以正确获取 bot 的密钥了
- 为脱水密钥恢复添加了解密
- 参照上游移除了旧的类型
- 向上游反馈了 patch (请升级到新版以完成主动回复与会话隔离功能的修复)
