...
def _coerce_record_component(segment) -> Record | None:
...
async def send_with_client_impl(
    ...
    for segment in chain_to_send:
        ...
        elif isinstance(segment, Record) or _matches_component_type(
            segment, ComponentType.Record
        ):
            try:
                record_segment = _coerce_record_component(segment)
