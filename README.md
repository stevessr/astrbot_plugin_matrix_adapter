...
- **多种认证方式**：支持密码认证、Access Token 认证、OAuth2 认证、扫码登录（QR）
...
| `matrix_auth_method` | string | `password` | 认证方式：`password`、`token`、`oauth2`、`qr` |
...
2. 检查密码或 Token 是否正确
3. 如果使用 OAuth2，确保服务器支持 `/_matrix/client/v1/auth_metadata` 或 `/.well-known/matrix/client`
