...
兼容不支持该提案的客户端仍会看到正常的编辑消息。

适配器会遵循房间状态 `org.matrix.msc4357.live_messaging`：当其内容为
`{"enabled": false}` 时，该房间自动退化为一次性普通回复；状态缺失时不额外禁用。
开启消息串时，初始 Live Message 会保留消息串关系，后续 `m.replace` 仍编辑该初始事件。
