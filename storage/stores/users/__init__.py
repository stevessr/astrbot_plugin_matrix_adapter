"""Composable Matrix interacted-user storage operations."""
...
from .cache import MatrixUserCacheMixin
from .profile import MatrixUserProfileMixin
from .storage import MatrixUserStorageMixin


class MatrixUserStore(
    MatrixUserStorageMixin,
    MatrixUserCacheMixin,
    MatrixUserProfileMixin,
):
...
for _mixin in (
    ...
    for _method_name, _method in _mixin.__dict__.items():
        if callable(_method) and not _method_name.startswith("__"):
            setattr(MatrixUserStore, _method_name, _method)
